...
function FoodItem({ food, onSelect }) {
  return (
    <button
      ...
      className="w-full bg-slate-700/50 hover:bg-slate-700 rounded-xl p-3 text-left transition-all active:scale-[0.98] flex items-center gap-3"
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          <span className="text-emerald-400 text-xs font-bold">{food.calories} ккал</span>
          <span className="text-slate-500 text-[10px]">на {food.weight_g}г</span>
        </div>
      </div>
...
function MealForm({ initial, onSave, onBack }) {
  ...
  const handleSave = () => {
    const name = (initial ? form.name : form.name).trim()
    if (!name) { toast.error('Введите название'); return }
    onSave(initial ? { ...scaled, name } : form)
  }

  const FIELDS = [
    { key: 'calories', label: 'Калории', unit: 'ккал', color: 'border-emerald-500' },
    { key: 'protein',  label: 'Белки',   unit: 'г',    color: 'border-red-400' },
    { key: 'fat',      label: 'Жиры',    unit: 'г',    color: 'border-yellow-400' },
    { key: 'carbs',    label: 'Углеводы',unit: 'г',    color: 'border-blue-400' },
  ]
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Portion slider (when from database) */}
      {initial && (
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-xs text-slate-400">Порция</label>
            <span className="text-emerald-400 font-bold text-sm">{Math.round(portion)} г</span>
          </div>
          <input
            type="range"
            min={10}
      ...
      <div className="grid grid-cols-2 gap-3">
        {FIELDS.map(({ key, label, unit, color }) => (
          <div key={key} className={clsx('bg-slate-700 rounded-xl p-3 border-l-4', color)}>
      ...
      {!initial && (
        <div>
          <label className="text-xs text-slate-400 mb-1 block">Вес порции (г)</label>
          <input
            type="number"
            value={form.weight_g}
      ...
      <div className="flex gap-3 pt-2">
        <button
          onClick={onBack}
          className="flex-1 py-3.5 bg-slate-700 text-white font-medium rounded-xl transition-all active:scale-95 border border-slate-600"
        >
          Назад
        </button>
        <button
          onClick={handleSave}
          className="flex-1 py-3.5 bg-emerald-500 hover:bg-emerald-400 text-white font-bold rounded-xl transition-all active:scale-95 flex items-center justify-center gap-2"
        >
          <Check size={18} />
          Сохранить
        </button>
...
export default function AddMealModal({ onClose }) {
  ...
  const [view, setView] = useState('search') // search | form
        ...
        {view === 'search' ? (
          <div className="space-y-4">
            ...
            <button
              ...
              className="w-full py-3 bg-slate-700/50 hover:bg-slate-700 text-slate-300 font-medium rounded-xl transition-all flex items-center justify-center gap-2 border border-dashed border-slate-600"
            >
            ...
            <div className="space-y-2 max-h-[40vh] overflow-y-auto">
              {results.length === 0 ? (
                <div className="text-center py-6">
                  ...
                  <button
                    onClick={handleManualEntry}
                    className="text-emerald-400 text-sm font-medium mt-2 hover:underline"
                  >
                    Ввести вручную
                  </button>
                </div>
                ...
                results.map((food, i) => (
                  <FoodItem key={i} food={food} onSelect={handleSelect} />
                ))
              )}
            </div>
          </div>
